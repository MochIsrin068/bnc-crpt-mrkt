      ...
      <section className="w-full" data-testid="mainTable">
         ...
         {mainContext?.cryptoAssets?.length === 0 ? (
            <div className="h-80 w-full flex flex-col items-center justify-center">
               <img src={ImageLoader} alt="Loader" className="h-16 w-16" />
            </div>
         ) : (
            mainContext?.cryptoAssets?.map((cryptoAsset, index) => <TableItem index={index} cryptoAsset={cryptoAsset} key={cryptoAsset.id} />)
         )}
      </section>
   )
}
